...
if USE_TILED_DATASET:
    DATASET_DIR = TILED_OUT_DIR
else:
    DATASET_DIR = SPLIT_OUT_DIR
...
# === Training parameters ===
MODEL_WEIGHTS = BASE_DIR / "runs" / "solar_v4_final_resplitretiled_100epochs" / "weights" / "best.pt"
EPOCHS = 50
IMGSZ = 640
BATCH = 4
DEVICE = "cuda"  # use NVIDIA GPU when available

# === Experiment tracking ===
RUNS_DIR = BASE_DIR / "runs"
RUN_NAME = "lebanon_generalization_eval"  
...
AUGMENT_CONFIG = {
    ...
    # Satellite imagery: horizontal flip usually OK
    "fliplr": 0.5,
    "flipud": 0.0,
...
# === Feature config for per-image relative diagnostics ===
# These settings are meant to be used by your diagnostics / post-processing script.
FEATURE_CONFIG = {
    # Relative-to-image normalization
    "use_relative_brightness": True,
    "use_relative_edge_density": True,

    # Brightness channel to use:
    # if using OpenCV BGR->HSV, V is brightness-like
...
# NOTE: these should be used in the diagnostics script, not interpreted as raw absolute rules.
OBJECT_FILTER_THRESHOLDS = {
    "min_relative_edge_density": 0.80,   # object edge density should not be far below image context
    "max_relative_brightness": 1.05,     # panels often darker or similar vs surrounding roof context
    "max_brightness_std_ratio": 1.10,    # more uniform than highly noisy backgrounds
